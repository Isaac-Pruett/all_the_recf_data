...
async def on_message(message: discord.Message):
    ...
    if cx.startswith("$summary"):
        ...
        await message.channel.send(
            f"""**{teamname} skills:** \t driver: `{driver_df.loc[teamname, "score"]}`, rank: `{driver_df.index.get_loc(teamname) + 1}` \t programming: `{programming_df.loc[teamname, "score"]}` rank: `{programming_df.index.get_loc(teamname) + 1}`
